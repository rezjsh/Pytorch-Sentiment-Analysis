...
def main():
    ...
    try:
        ...
        preprocessed_data_tuple, raw_data, vocab_size = data_preprocessing_pipeline.run_pipeline()
        ...
        # eda_pipeline.run_pipeline(raw_data=raw_data)
        # logger.info(f">>>>>> stage {STAGE_NAME} completed <<<<<<\n\n")


        # Decide which pipeline to run based on model type
        if config_manager.get_mode() == 'ML':
            run_ml_branch(config_manager, raw_data)
        elif config_manager.get_mode() == 'DL':
            run_dl_branch(config_manager, preprocessed_data_tuple, vocab_size)
        else:
...
def run_dl_branch(config_manager, preprocessed_data_tuple, vocab_size=None):
    ...
    logger.info(
        f"Final DataLoaders created. Train batches: {len(train_loader)}, Validation batches: {len(val_loader)}, Test batches: {len(test_loader)}"
    )
    logger.info(f">>>>>> stage {STAGE_NAME} completed <<<<<<\n\n")
    ...
    model = model_pipeline.run_pipeline(vocab_size=vocab_size)
    logger.info(f">>>>>> stage {STAGE_NAME} completed <<<<<<\n\n")
    ...
    # STAGE_NAME = "Stage 06: Model Training"
    # logger.info(f">>>>>> stage {STAGE_NAME} started <<<<<<")
    ...
    STAGE_NAME = "Stage 07: Model Evaluation"
    logger.info(f">>>>>> stage {STAGE_NAME} started <<<<<<")
...
def run_ml_branch(config_manager, raw_data):
    ...
    ml_pipeline.run_pipeline(X_train, y_train, X_test, y_test)
