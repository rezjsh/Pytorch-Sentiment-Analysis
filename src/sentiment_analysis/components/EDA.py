...
class SentimentEDA:
    ...
    def plot_word_frequency(self, texts: List[str], file_name: str, title: str, max_words: int = None):
        ...
        if not cleaned_words:
            logger.warning(f"No cleaned words found for '{title}'. Skipping word frequency plot.")
            return pd.DataFrame(columns=['word', 'count', 'relative_freq'])

        word_counts = Counter(cleaned_words).most_common(max_words)
        if not word_counts:
            logger.warning(f"No common words found for '{title}'. Skipping word frequency plot.")
            return pd.DataFrame(columns=['word', 'count', 'relative_freq'])

        total_words = sum(count for _, count in word_counts)

        # Defensive check for total_words, though previous checks should catch it
        if total_words == 0:
            logger.warning(f"Total words count is zero for '{title}'. Skipping relative frequency calculation.")
            return pd.DataFrame(columns=['word', 'count', 'relative_freq'])
    ...
    def generate_word_cloud(self, texts: List[str], file_name: str, title: str):
        ...
        if not cleaned_text.strip():
            logger.warning(f"No cleaned text found for '{title}'. Skipping word cloud generation.")
            return
    ...
    def run_full_eda(self, raw_texts: List[str], raw_labels: List[int],
        ...
        # Ensure pos_wc and neg_wc are DataFrames even if no words were plotted
        pos_wc_df = pos_wc if isinstance(pos_wc, pd.DataFrame) else pd.DataFrame(columns=['word', 'count', 'relative_freq'])
        neg_wc_df = neg_wc if isinstance(neg_wc, pd.DataFrame) else pd.DataFrame(columns=['word', 'count', 'relative_freq'])
        self.save_summary_stats(df_train, pos_wc_df, neg_wc_df)
